...
exports.default = async (argv) => {
    ...
    mkdirp_1.mkdirp.sync(tmp);
    process.chdir(tmp);
    (0, shelljs_1.exec)(`npm install ${getPackages(pkg)} --production --prefix ./protobufs`);
    // protos
    const pkgs = (0, glob_1.sync)('./protobufs/**/package.json');
    const cmds = pkgs
        ...
        return [extDir, (0, path_1.resolve)((0, path_1.join)(cur, 'proto', dst)), dir];
    });
    // move protos 
    for (const [src, dst, pkg] of cmds) {
        (0, rimraf_1.rimrafSync)(dst);
        console.log(`installing ${pkg}...`);
        mkdirp_1.mkdirp.sync((0, path_1.dirname)(dst));
        (0, shelljs_1.exec)(`mv ${src} ${dst}`);
    }
    // package
    const packageInfo = JSON.parse((0, fs_1.readFileSync)('./protobufs/package.json', 'utf-8'));
